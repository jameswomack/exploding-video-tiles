...
// Opportunity to use `import`?
require('./analyzeColors');
require('./keyboard');


(function () {
  // Opportunity to use `let` or `const` ?
  ...
  // Plenty of opportunities for `const` here
  var pointilismPixelState = { }
  var Draw = { }
  var Metrics = { }
  var Cache = { }
  var tick = null

  var FRAME_RATE = 23.96 // Roughly NTSC
  ...
  function getCurrentFrame () {
    var key = videoEl.currentTime.toFixed(5)
  ...
  // Opportunity to use `class` & default params?
  ...
  // Opportunity to use string template literals and/or default params?
  ...
  Draw.ellipses = function (tile, ctx) {
    ...
    var swidth = Math.max(1, Math.floor(Metrics.currentTileMode.TILE_WIDTH*(videoRatio*10)-tile.videoX))
    var sheight = Math.max(1, Math.floor(Metrics.currentTileMode.TILE_HEIGHT*(videoRatio*10)-tile.videoY))
    var dx = Math.max(1, Math.floor(-(videoRatio*10)+tile.currentX-Metrics.currentTileMode.TILE_WIDTH/2))
    var dy = Math.max(1, Math.floor(-(videoRatio*10)+tile.currentY-Metrics.currentTileMode.TILE_HEIGHT/2))
  ...
  Draw.pointillism = function (tile, ctx) {
    ...
    var currentPixel = Cache.pointillism[currentFrame][tile.currentX][tile.currentY] || (Cache.pointillism[currentFrame][tile.currentX][tile.currentY] = getPixel(tile, copyCtx))
  ...
  function updateTileMode () {
    var currentTileModeNameFromLocation = location.hash.replace('#', '')
    currentTileModeName = currentTileModeNameFromLocation || currentTileModeName

    var tileModeUnInitialized = !Metrics.currentTileMode
    var tileModeOutOfSync = tileModeUnInitialized ? true : Metrics.currentTileMode.NAME !== currentTileModeName
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Opportunity to use `NodeList.prototype[Symbol.iterator]` ?
    // `Array.from` ?
    ...
    setInterval(function () {
      if (!videoEl.paused){
        var image = copyCtx.getImageData(100, 100, copyCtx.canvas.width/2, copyCtx.canvas.height/2);
        // Opportunity to use object literal shorthand?
        worker.postMessage({ image : image })
      }
    }, 1000)

    document.addEventListener('keypress', function (e) {
      window.matchesKeyCodes(e, [ 112, 32 ]) && window.isNotKeyCombo(e) &&
